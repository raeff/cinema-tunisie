...
exports.delete = function (showId) {
    const promise = ShowTime.findByIdAndRemove(showId)
    if (!promise) return res.status(404).send('The show with the given ID was not found.');
    return promise;
};


exports.update = function (showtime) {
    const promise = ShowTime.findOneAndUpdate({ _id: showtime._id }, showtime).exec();
    return promise;
};
