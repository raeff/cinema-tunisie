...
exports.updateShowtime = function (req, res) {
    const showtime = Object.assign({}, req.body);
    const resolve = (showtime) => {
        res.status(200);
        res.json(showtime);
    };
    showtime._id = req.params.showtimeId;
...
exports.show_remove = function (request, response) {
    const resolve = (show) => {
        response.status(200);
        response.json(show);
    };
    showtimeService.delete(request.params.showId)
        .then(resolve)
